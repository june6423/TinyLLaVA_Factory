DATA_PATH=/data1/dongjun/dataset/text_files/blip_laion_cc_sbu_558k.json #pretrain annotation file path
FINETUNE_DATA_PATH=/data1/dongjun/dataset/text_files/llava_v1_5_mix665k.json #finetune annotation file path
IMAGE_PATH=/data1/dongjun/dataset/llava/llava_pretrain/images #pretrain image dir
FINETUNE_IMAGE_PATH=/data1/dongjun/dataset #finetune image dir
...
bash scripts/train/custom_finetune.sh "$FINETUNE_DATA_PATH" "$FINETUNE_IMAGE_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$CONV_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH"
